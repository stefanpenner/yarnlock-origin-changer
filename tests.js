...
describe("yarnlock-hostname-changer", function () {
  it("has a working replacer", function () {
    ...
    const { result, total, replaced } = replacer(
      original,
  ...
  it("supports replacing an existing pathname", function () {
    ...
    expect(original).to.contain(
      "http://registry.iamstef.net/api/npm/@yarnpkg/lockfile/-/lockfile-1.1.0.tgz#e77a97fbd345b76d83245edcd17d393b1b41fb31"
    );
  ...
  it("test CLI", function () {
    ...
    fs.writeFileSync(
      `${TMP}/yarn.lock`,
      fs.readFileSync(`${FIXTURES}/yarn.lock`)
    );
